...
namespace Drivers.Security
{
    ...
    public static class SecurityManager
    {
        ...
        private const int ENCRYPTION_KEY_BITS = 256;
        private const int ENCRYPTION_ITERATIONS = 1000;
        ...
        public static string CreateHash(string unHashedString)
        {
            ...
            byte[] hash = PBKDF2(unHashedString, salt, PBKDF2_ITERATIONS, HASH_BYTES);
        ...
        public static bool VerifyHash(string unHashedString, string correctHashedString)
        {
            char[] delimiter = { ':' };
            string[] split = correctHashedString.Split(delimiter);

            if (split.Length != HASH_SECTIONS)
            {
                throw new InvalidHashException(
                    "Fields are missing from the hashed string."
                );
            }
            ...
            byte[] testHash = PBKDF2(unHashedString, salt, iterations, hash.Length);
